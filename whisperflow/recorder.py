...
whatever the device accepts and convert to 16 kHz mono in stop().
"""
...
class Recorder:
    ...
    def stop(self) -> np.ndarray:
        ...
        if self._rate != SAMPLE_RATE and audio.size:
            target_len = int(audio.size * SAMPLE_RATE / self._rate)
            audio = np.interp(
