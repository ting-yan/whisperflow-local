...
- Settings window: pick your microphone, hotkey, and model size
- System-tray icon with live status (blue = ready, red = recording, orange = transcribing)
- Custom vocabulary so your names/jargon come out spelled right
...
- **Hold F8**, speak, release. High beep = recording, low beep = processing.
  The text pastes at your cursor; your clipboard is restored afterwards.
...
Flow: `hold hotkey → record mic → release → Whisper transcribe → format → paste`
...
| Model | `tiny.en` (fastest) → `large-v3` (best). `base.en`/`small.en` are the sweet spots on CPU |
| Vocabulary | Comma-separated words Whisper should favor (names, jargon) |
| AI cleanup | Sends transcripts to Claude (`claude-haiku-4-5`) for grammar/filler fixes. Needs `ANTHROPIC_API_KEY`; no longer fully local when enabled |

Config-file-only options: `language` (`null` = auto-detect on multilingual
models), `paste_mode` (`"type"` simulates keystrokes for apps that block
paste), `device`/`compute_type` (`"cuda"`/`"float16"` for NVIDIA GPUs with
the CUDA 12 runtime: `pip install nvidia-cublas-cu12 nvidia-cudnn-cu12`).
